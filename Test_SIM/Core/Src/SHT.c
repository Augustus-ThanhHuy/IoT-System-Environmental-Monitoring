...
SHT31_Status SHT31_GetData(Mode mode, Repeatability rep, ClockStretch stretch, MPS mps)
{
    ...
    {
        if (HAL_GetTick() - sht31.lastMeasureTime > MilsPerMeasure[mps])
        { // must wait for enough time before read in Periodic mode
            sht31.lastMeasureTime = HAL_GetTick();

            if (sht31.isCurrentlyPeriodic == 0)
            { // Just the first period in Periodic mode need to send command
