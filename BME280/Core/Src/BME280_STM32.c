...
void BME280_Measure(void)
{
	...
	if (BMEReadRaw() == 0)
	{
		// Xử lý nhiệt độ
		if (tRaw == 0x800000)
			Temperature = 0; // Nếu đo nhiệt độ bị tắt
		else
			Temperature = (BME280_compensate_T_int32(tRaw)) / 100.0; // Tính toán nhiệt độ

		// Xử lý áp suất
		if (pRaw == 0x800000)
			Pressure = 0; // Nếu đo áp suất bị tắt
		else
		{
#if SUPPORT_64BIT
			Pressure = (BME280_compensate_P_int64(pRaw)) / 256.0; // Tính toán áp suất cho 64 bit
#elif SUPPORT_32BIT
			Pressure = (BME280_compensate_P_int32(pRaw)); // Tính toán áp suất cho 32 bit
#endif
		}

		// Xử lý độ ẩm
		if (hRaw == 0x8000)
			Humidity = 0; // Nếu đo độ ẩm bị tắt
		else
			Humidity = (bme280_compensate_H_int32(hRaw)) / 1024.0; // Tính toán độ ẩm
...
{
	// Đọc chip ID
	HAL_I2C_Mem_Read(&hi2c1, BME280_ADDRESS, ID_REG, 1, &chipID, 1, HAL_MAX_DELAY);

	// In ra giá trị chip ID
	printf("Chip ID: 0x%02X\n", chipID);

	// Kiểm tra giá trị chip ID
	if (chipID == 0x60 || chipID == 0x58)
	{
		return 0; // Cảm biến hợp lệ
	}
	else
	{
		return -1; // Cảm biến không hợp lệ
	}
}
